...
class ItineraryAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    fun setItinerary(itinerary: Map<String, List<ItineraryItem>>) {
        ...
        expandedDays = itinerary.keys.toMutableSet()
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        for (day in dayOrder) {
            if (pos == position && holder is DayHeaderViewHolder) {
                // Format day label as 'Day 1', 'Day 2', etc.
                val dayNumber = day.filter { it.isDigit() }
                holder.dayTextView.text = "Day $dayNumber"
            ...
            if (expandedDays.contains(day)) {
                ...
                if (position < pos + items.size) {
                    ...
                    if (holder is ItineraryViewHolder) {
                        // If using MaterialChip for time, hide if empty
                        if (holder.timeTextView is com.google.android.material.chip.Chip) {
